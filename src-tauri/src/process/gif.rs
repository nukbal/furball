...
pub async fn convert(file_path: String, config: Config) -> Result<(), String> {
  let dir_path = super::videos::frames(file_path.clone()).await?;
  ...
  let _vid_out = super::external::ffmpeg([
    "-i",
    out_path.clone().join("frame-%d.png").to_str().unwrap(),
