...
pub const Models = struct {
    ...
    pub fn init() Models {
        return .{
            .x2 = .{ .param = x2_param, .model = x2_model, .scale = 2 },
            .x4 = .{ .param = x4_param, .model = x4_model, .scale = 4 },
        };
    }
    ...
    pub fn upscale(
        ...
        const scale = required_scale / short_edge;
        const model = if (scale <= 2) &self.x2 else &self.x4;

        return model.upscaleOnce(io, allocator, input_rgb, input_width, input_height, output_width, output_height);
    }
};
...
const Model = struct {
    ...
    fn upscaleOnce(
        ...
        const output_width_u64 = std.math.mul(u64, input_width, self.scale) catch return error.InvalidResize;
        const output_height_u64 = std.math.mul(u64, input_height, self.scale) catch return error.InvalidResize;
        ...
        const tile_span_output = std.math.mul(u64, tile_span, self.scale) catch return error.InvalidResize;
        ...
        while (y < input_height) : (y += tile_size) {
            ...
            while (x < input_width) : (x += tile_size) {
                ...
                const expected_tile_width = std.math.mul(u32, tile_width, self.scale) catch return error.InferenceFailed;
                const expected_tile_height = std.math.mul(u32, tile_height, self.scale) catch return error.InferenceFailed;
                ...
                const crop_left = std.math.mul(u32, x - tile_x, self.scale) catch return error.InvalidResize;
                const crop_top = std.math.mul(u32, y - tile_y, self.scale) catch return error.InvalidResize;
                const core_width_output = std.math.mul(u32, core_width, self.scale) catch return error.InvalidResize;
                const core_height_output = std.math.mul(u32, core_height, self.scale) catch return error.InvalidResize;
                ...
                for (0..core_height_output) |row| {
                    ...
                    const destination_offset = try outputOffset(output_width_value, self.scale, x, y, row);
