...
pub fn convertMp4ToGif(alloc: Allocator, io: std.Io, source: []const u8, destination: []const u8) !void {
    ...
    const source_stat = std.Io.Dir.cwd().statFile(io, source, .{ .follow_symlinks = false }) catch return error.SourceNotFound;
...
test "video rejects paths containing nul bytes" {
    try std.testing.expectError(error.InvalidPath, convertMp4ToGif(std.testing.allocator, std.testing.io, "clip\x00.mp4", "clip.gif"));
    try std.testing.expectError(error.InvalidPath, convertMp4ToGif(std.testing.allocator, std.testing.io, "clip.mp4", "clip\x00.gif"));
}
